...
unsigned long turnSignalTime = 0;
unsigned char TurnSigs[8] = {0,0,0,0,0,0,0,0};

uint8_t coolantByte;

unsigned long lastCAN = 0;
const unsigned long CAN_INTERVAL = 15;
...
#define DEVICE_UNIQUE_ID "50d263fc-534a-483d-a2de-c206a393e9e8" //{"UniqueId":"50d263fc-534a-483d-a2de-c206a393e9e8","Name":"DEVICE_UNIQUE_ID","Type":"uniqueid"}
...
void setup()
{
 ...
 //cluster stuff for warning lights
...
void warningLEDs()
{
  if (shCustomProtocol.handbrake > 0)
  {
    digitalWrite(4, HIGH);
  }
  else
...
void sendCAN() {
  ...
  FlowSerialDebugPrintLn("ino: " + String(shCustomProtocol.ignition));
  if (shCustomProtocol.ignition > 0)
  {
    CanSend(0x575, 0x07,0x00,0x00,0x00,0x00,0x00,0x00,0x00);//ignition 
    CanSend(0x480, 0,0,0,0,0,0,0,0); // epc light off
    CanSend(0x3D0, 0, 0, 0, 0, 0, 0, 0, 0);// power steering light off
  }
  else
  {
    CanSend(0x575, 0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00);
  }

  byte speedRaw = shCustomProtocol.speed / 1.875;
  CanSend(0x5A0, speedRaw, speedRaw, speedRaw, speedRaw, speedRaw, speedRaw, speedRaw, speedRaw); //speed i could never be bothered to sort out the bytes

  //RPM
  tempRPM = shCustomProtocol.rpm*4;
  rpmL = lo8(tempRPM);
  rpmH = hi8(tempRPM);
  CanSend(0x280, 0x49, 0x0E, rpmL, rpmH, 0x0E, 0x00, 0x1B, 0x0E);

  //coolant need to look at again later kinda got it look at the refining copy sketch

  //turn signal
  if(shCustomProtocol.turnLeft > 0 && shCustomProtocol.turnRight > 0 && millis() - turnSignalTime >= 1000)
  {
    TurnSigs[2] = B00000011;
    turnSignalTime = millis();
  }
  else if (shCustomProtocol.turnLeft > 0 && millis() - turnSignalTime >= 1000)
  {
    TurnSigs[2] = B00000001;
    turnSignalTime = millis();
  }
  else if (shCustomProtocol.turnRight > 0 && millis() - turnSignalTime >= 1000)
  {
    TurnSigs[2] = B00000010;
    turnSignalTime = millis();
  }

  if (turnSignalTime > 0) {
      if (millis() - turnSignalTime >= 500) {  
          TurnSigs[2] = B00000000;                        
      }
  }
  if (turnSignalTime > 0) {
      if (millis() - turnSignalTime >= 1000) {  
        turnSignalTime = 0;          
      }
  }

  //high beams, the cluster has no low beam light i think
  if(shCustomProtocol.highBeam > 0)
  {
    TurnSigs[0] = B00000100;  
  }
  else
  {
    TurnSigs[0] = B00000000;  
  }

  //send the highbeams and turn signals
  CanSend(1329, TurnSigs[0], TurnSigs[1], TurnSigs[2], TurnSigs[3], TurnSigs[4], TurnSigs[5], TurnSigs[6] ,TurnSigs[7]);
  
  //water temp
  if (shCustomProtocol.coolant < 50) {
  coolantByte = 0x70;
  }
  else if (shCustomProtocol.coolant < 80) {
    coolantByte = map(shCustomProtocol.coolant, 50, 80, 0x80, 0xA2);
  }
  else if (shCustomProtocol.coolant < 95) {
    coolantByte = map(shCustomProtocol.coolant, 80, 95, 0xA2, 0xC0);
  }
  else if (shCustomProtocol.coolant < 120) {
    coolantByte = map(shCustomProtocol.coolant, 95, 120, 0xC0, 0xE0);
  }
  else {
    coolantByte = 0xEA; // overheat
  }
  CanSend(0x288, 0, coolantByte, 0, 0, 0, 0, 0, 0);
